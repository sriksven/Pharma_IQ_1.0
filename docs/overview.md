...
- **Database Engine**: DuckDB (An incredibly fast, in-memory analytical database for querying CSVs).
...
**What it does:** 
On server startup (inside `app/main.py`), the pipeline scans the raw `.csv` directory. It uses `pandas` to infer data types, registers each CSV as a highly-efficient **DuckDB View**, and builds a `registry.json` file. This JSON registry maps out the entire schema (columns, types, and auto-detected foreign-key relationships) so it can be injected into the LLM prompt.
...
## 5. UI, Backend, and "Gold" Evaluations
...
Because SQL correctness doesn't always guarantee a *good human answer*, the system runs a background async task. It takes the user's prompt, the generated SQL, and the final English answer, and passes them to a secondary "LLM Judge" to score them exclusively from 1 to 10:
...
* **How & Why we use it:** Before merging any new code to `main` (or swapping the Groq LLM for a newer model), developers run the Gold Eval. It executes the entire chat pipeline against all questions and compares its generated SQL against the verified Gold SQL. If accuracy drops below 95%, the build fails. It guarantees that we never accidentally deploy a software update that corrupts how `fact_rx.csv` joins to `hcp_dim.csv`.
